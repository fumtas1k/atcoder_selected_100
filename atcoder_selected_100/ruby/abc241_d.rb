...
sorted = QUERY.filter_map { _1[1] if _1[0] == 1 }.sort.uniq
sorted_set = FwtSortedMultiSet.new(sorted)
ans = []
QUERY.each do |query|
  case query
  in [1, x]
    sorted_set.add(x)
  in [2, x, k]
    rank = sorted_set.upper_bound(x) - 1
    target_rank = rank - k + 1
    key = sorted_set.kth(target_rank)
    ans << (key.nil? ? -1 : key)
  in [3, x, k]
    rank = sorted_set.lower_bound(x)
    target_rank = rank + k - 1
    key = sorted_set.kth(target_rank)
    ans << (key.nil? ? -1 : key)
  end
